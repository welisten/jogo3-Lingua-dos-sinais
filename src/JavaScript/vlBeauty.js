window.VLibras = function(t) {
	...
	(e = n(1)(!1)).push([t.i, "[vw] [vw-access-button]{display:none;flex-direction:row;width:40px;height:40px;cursor:pointer;overflow:hidden;position:relative;border-radius:8px;transition:all 1s ease;right:0;left:auto}[vw] [vw-access-button] img{max-height:40px;transition:all .5s ease;border-radius:8px;opacity:1 !important;visibility:visible !important}[vw] [vw-access-button] .access-button{width:40px;height:40px;z-index:1}[vw] [vw-access-button].isLeft{flex-direction:row;left:0;right:auto}[vw] [vw-access-button].isLeft .pop-up{left:auto;right:0}[vw] [vw-access-button].isTopOrBottom:hover{bottom:-20px;top:0;margin-right:-80px}[vw] [vw-access-button].active{display:flex}\n", ""]), t.exports = e
}, function(t, e, n) {
